...
##################################### FUNCTIONS_METHODS.R ######################################## 
## This file contains the functions required for executing the simulation study in Chapter 4:   ##
##    - simulate_data                                                                           ##                                                                          
##    - create_subset                                                                           ## 
##    - generate_script                                                                         ##
...
create_subset = function(folder, iteration, ind, cov, N, ME) {

  #If a certain simulated data set does not exist in the global environment, create it 
...
##################################################################################################
## Generate a Latent Gold script for estimating an LC model. Note that this function can be     ##  
## used to estimate regular LC models, as well as the LC models in LCT and tree-MILC analysis.  ##                         
## @param type (string): What type of model (e.g. "LC" for regular LC and "LCT" for LCT step 2) ## 
## @param ind (int): Number of indicators                                                       ##
## @param cov (int): Number of covariates (e.g. 0 or 1)                                         ##
...
generate_script = function(type, ind, cov, N, model_name, filepath_input, filepath_output){
  ...
  #The number of starting values depends on the size of the data set
  if(N < 10000){
    sets = 3200
  } else {
    sets = 100
  ...
  #For LCT step 2 and four indicators, add restriction that Y2(2) < Cluster(2) = 0 and Y4(2) < Cluster(2) = 0 to avoid boundary solutions 
  if((type == "LCT") & ind == 4){
    restrictions = "\n\tx[1,1] = 0; y[1,1] = 0;"
  } else { 
    restrictions = ""
...
## posterior probabilities associated with response patterns that were not present in the       ##
...
generate_script_treeMILC_extra = function(type, ind, cov, N, model_name, filepath_input, filepath_output, par){
...
fix_cluster_bootstrap = function(boot_results) {
  ...
  #Compute a measurement error probability matrix (i.e. compute a contingency table for the values in indicator Y2 and the imputed values and normalise by rows)
...
fix_number_notation = function(vector) {
  ...
  #If the vector is not numeric, correct values in the wrong format
  } else {
    ...
    for (i in 1:length(vector)) {
      ...
      #If the string did not contain 'e'
        ...
        if (temp_string == ".") {
          return_vec[i] = 0  
        #In other cases, simply convert to numeric
        } else { 
...
perform_lc = function(iteration, ind, cov, N, ME, dat = NULL, folder) {
  ...
  #Create subset
  if (is.null(dat)) {
    dat = create_subset(folder, iteration, ind, cov, N, ME)
  } else {
    dat = dat
  }
  ...
  script = generate_script("LC", ind, cov, N, model_name, filepath_input, filepath_output)
...
perform_lct = function(iteration, ind, cov, N, ME, folder) {
  ...
  script = generate_script("LCT", ind, cov, N, model_name, filepath_input, filepath_output)
...
perform_treeMILC = function(iteration, ind, cov, N, ME, M=5, folder) {
  ...
  dat_org = create_subset(folder, iteration, ind, cov, N, ME)
  ...
  for (i in 1:M) {
    ...
    if (nrow(count_boot) != nrow(count_dat)) {
      ...
      remove = which(grepl("(1)", parameters$term1, fixed = TRUE, useBytes = TRUE) | grepl("(1)", parameters$term3, fixed = TRUE, useBytes = TRUE)) #Remove dummies that are 0
      ...
      script = generate_script_treeMILC_extra("LC", ind, cov, N, model_name, filepath_input = dat_org_path, filepath_output = output_path, parameters)
    ...
    script = generate_script("treeMILC", ind, cov, N, model_name, filepath_subset, filepath_output)
