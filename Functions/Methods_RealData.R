################################### Methods_RealData.R ###########################################
## This file contains the functions required to perform LC and tree-MILC analysis on            ##
## real data from the ER and the LFS (see Chapter 6). This file contains the functions          ##
##    - perform_lc                                                                              ##
##    - perform_treeMILC                                                                        ##
##################################################################################################
...
perform_lc = function(cov_ok, cov_problem, dat = NULL, dat_path = NULL, folder){
  ...
  #If data set is provided, keep only relevant columns
  } else {
    dat = dat[,which(colnames(dat) %in% c("contract","contractEBB","persnr",cov_ok,cov_problem))]
...
perform_treeMILC = function(cov_ok, cov_problem, dat = NULL, dat_path = NULL, folder, M=5){
  ...
  for(i in 1:M){
    ...
    sample$persnr = row.names(sample) #Change id to make sure it captures duplicates
    ...
    count_boot = as.data.frame(boot_output[,-which(colnames(boot_output) %in% c("persnr","cluster"))] 
    ...
    count_step2 = as.data.frame(model2_output[,-which(colnames(boot_output) %in% c("persnr","Cluster#"))] 
                                %>% group_by_all() %>% summarise(COUNT = n()))
