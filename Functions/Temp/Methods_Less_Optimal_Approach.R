############################# Methods_Less_Optimal_Approach.R ####################################
...
## 5.1.1). The current file contains the following functions:                                   ##
##    - generate_script                                                                         ##
##    - perform_lc                                                                              ##
##    - perform_lct                                                                             ##
##    - perform_treeMILC                                                                        ##
...
generate_script <- function(type, ind, cov_ok, cov_problem, N, model_name, filepath_input, filepath_output, start_val = "") {
  ...
  script <- paste0(script_part1, model_name, script_part2, script_part3, caseweight, dep_ind, dep_cov,
                   latent_var, latent_var_eq, dep_ind_eq, start_val, restriction, "\nend model")
  return(script)
}
...
perform_lc <- function(iteration, ind, cov_ok, cov_problem, N, ME, dat = NULL, folder) {
  ...
  # Assign observations with missing covariates to the first covariate category
...
perform_lct <- function(iteration, ind, cov_ok = NULL, cov_problem = NULL, N, ME, folder) {
  ...
  # Check if the model is valid (i.e., does not contain a warning)
...
perform_treeMILC <- function(iteration, ind, cov_ok, cov_problem, N, ME, M = 5, folder) {
  ...
  # Count combinations of indicators and (all) covariates in the original data set
  ...
  # Count combinations of indicators and (only) non-missing covariates in the original data set
  ...
  for (i in 1:M) {
    ...
    # Perform LC with 3 classes without missing covariates
    boot_output <- perform_lc(iteration, ind, cov_ok, cov_problem = NULL, N, ME, dat = sample, folder = folder)
    
    # If model contains an error, add error to model output and return
    ...
    # Get data frame with model results (i.e. ignore [[1]] model information)
    boot_output <- boot_output[[2]]
    
    # Count combinations of indicators and non-missing covariates in the bootstrap sample
    count_boot <- as.data.frame(boot_output[, -which(colnames(boot_output) %in% c("id", "cluster"))] %>% group_by_all() %>% summarise(COUNT = n()))
    ...
    if (nrow(count_boot) != (nrow(count_dat_cov_ok))) {
      ...
      if (length(cov_problem) == 2) {
        model_name2 <- gsub("null-null", "null", model_name2)
      }
      
      # Get data frame with parameters from earlier model
      ...
      for (k in 1:nrow(parameters)) {
        if (!is.na(parameters[k, ]$coef) & parameters[k, ]$coef != 0) {
          start_val <- paste(start_val, parameters[k]$coef)
        }
      }
      start_val <- paste(start_val, "}")
      ...
      # Use model output to assign posterior probabilities to every observation in the original data set
      ...
      dat <- fix_cluster_assignment(type = "LC", list(1, dat)) # Fix cluster assignment
      dat <- dat[[2]]
    ...
    # Use results from the second model to assign posterior probabilities to every observation in the data set
    count_step2 <- as.data.frame(model2_output[, -which(colnames(model2_output) %in% c("id", "Cluster#"))] %>% group_by_all() %>% summarise(COUNT = n()))
