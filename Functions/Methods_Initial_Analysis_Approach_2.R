...
## This file contains the functions required to perform the initial analysis in Section 5.1.2.  ##
## This analysis involves performing LCT and tree-MILC analysis using the second approach to    ## 
...
generate_script <- function(type, ind, cov_ok, cov_problem, N, model_name, dat, filepath_input, filepath_output, start_val = NULL) {
  ...
  script <- paste0(script_part1, model_name, script_part2, script_part3, caseweight, dep_ind, dep_cov,
                   latent_var, latent_var_eq, dep_ind_eq1, dep_ind_eq2, restrictions1, restrictions2, start_val, "\nend model")
...
perform_treeMILC <- function(iteration, ind, cov_ok, cov_problem, N, ME, M = 5, folder) {
  ...
  for (i in 1:M) {
    ...
    if (nrow(count_boot) != nrow(count_dat)) {
      ...
      parameters <- suppressWarnings(fread(parameters_path, sep = "\t", dec = ","))[, 1:7]
      names(parameters) <- c("term1", "term2", "term3", "term4", "term5", "coef", "SE")
      ...
      script <- generate_script("LC",ind, cov_ok, cov_problem, N, model_name, dat = dat,
    ...
    script <- generate_script("treeMILC", ind = ind, cov_ok = cov_ok, cov_problem = cov_problem, N = N, model_name = model_name, dat = subset, filepath_input = filepath_subset, filepath_output = filepath_output)
