...
setwd("F:/Documents/Thesis/Schrijfbestanden/Plot_Sim_8/heatmaps/")
...
for (j in c(1000, 10000)) {
  for(m in cov_problem){
    for (k in ind) {
      ...
      heatmap <- get_ME_heatmap(LC_models, LCT_models, treeMILC_models, LC_results, 1, k, cov_ok, m, j)
      ...
      heatmap <- get_ME_heatmap(LC_models, LCT_models, treeMILC_models, LC_results, 2, k, cov_ok, m, j)
      filename <- paste0(name, "_Variance_10-20-30.pdf")
      ggsave(plot = heatmap, width = 3.9, height = 3, dpi = 300, filename = filename)
      
      # Plot bias for models with a realistic 7% ME
      heatmap <- get_ME_heatmap_realistic(LC_models, LCT_models, treeMILC_models, LC_results, 1, k, cov_ok, m, j)
