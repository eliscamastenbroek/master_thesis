...
## in Chapter 5. To create most plots, the following objects are required to be in the global   ##
## environment:                                                                                 ##
...
names(plot_df)[which(names(plot_df) == "cov_problem")] <- "C"
...
plot_df[plot_df$N==n,] %>% ggplot(aes(x=indicator, y=rmse, fill=type)) +
  ...
  facet_grid(Contract + cov_problem ~ ME, labeller=labeller(.rows=label_value, .cols=label_both)) +
  theme(legend.position="top")
...
plot_df[plot_df$N==1000 & plot_df$Contract=="Permanent",] %>% ggplot(aes(x=indicator, y=prop, fill=type)) +
  ...
  facet_grid(cov_problem ~ ME,labeller=label_value) +
...
plot_ME[plot_ME$N==n,] %>% ggplot(aes(x=indicator, y=ME1, fill=type)) +
  ...
  facet_grid(Contract+cov_problem ~ ME,labeller=label_value) +
...
plot_df[plot_df$type!="tree-MILC",] %>% ggplot(aes(x=indicator, y=entropy, fill=type)) +
  ...
  facet_grid(ME ~ N + cov_problem, labeller=label_value) + 
  theme(legend.position="top")
...
ind <- 2:3
cov_problem <- c("NULL","baanduur", "SBIgroep", c("baanduur","SBIgroep"))
