...
generate_script_treeMILC_step1 <- function(type, ind, cov_ok, cov_problem, model_name, dat, filepath_input, filepath_output, par) {
  ...
  # Create vectors with letters to use for restrictions later on
  letters <- c("aa", "bb", "cc", "dd", "ee", "ff", "gg", "hh", "ii", "jj")
  letters_count <- 0
  ...
  caseweight <- if (type == "LCT") "caseweight p1;\n" else ""
  
  # Adjust equations depending on the number of indicators
  dep_ind <- paste0("\tdependent Y", 1:ind, " nominal;")
  dep_ind_eq2 <- paste0("\n\tY", 2:ind, " <- (kk) 1 | Cluster;")
  ...
  if (!is.null(cov_problem)) {
    for (i in 1:length(cov_problem)) {
      ...
      for (j in 1:num_cats) {
        if (j != num_cats) {
          restrictions2 <- paste0(restrictions2, "\n\t", letters[i], "[", j, ",] = 0;") # Specify the second restriction (see Section 5.1)
        } else {
          restrictions2 <- paste0(restrictions2, "\n\t", letters[i], "[", j, ",1] = -100;") # Specify the first restriction (see Section 5.1)
          restrictions2 <- paste0(restrictions2, "\n\t", letters[i], "[", j, ",2] = 0;")
          restrictions2 <- paste0(restrictions2, "\n\t", letters[i], "[", j, ",3] = -100;")
        }
      }   
  ...
  if (!is.null(cov)) {
    for (i in 1:length(cov)) {
      ...
      num_cats <- length(levels(factor(dat[, which_col])))
      letters_count <- letters_count + 1
      latent_var_eq <- paste0(latent_var_eq, " + (", letters[letters_count], ") ", cov[i])
      par2 <- (num_cats - 1) * 2
      
      for (j in 1:par2) {
        df_counter <- df_counter + 1
        if (par[df_counter,]$coef == 0) {
          restrictions <- paste0(restrictions, "\t", letters[letters_count], "[1,", j, "] = ", par[df_counter,]$coef, ";\n")
        } else {
          restrictions <- paste0(restrictions, "\t", letters[letters_count], "[1,", j, "] ~= ", par[df_counter,]$coef, ";\n")
        }
      }
...
perform_treeMILC <- function(iteration, ind, cov_ok, cov_problem, N, ME, M = 5, folder) {
  ...
  for (i in 1:M) {
    ...
    if (nrow(count_boot) != nrow(count_dat)) {
      ...
      script <- generate_script_treeMILC_step1("LC", ind, cov_ok, cov_problem, model_name, dat = dat,
