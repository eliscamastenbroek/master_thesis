...
get_ME_heatmap = function(ME_estimated, ME_HMM){
  ...
  plot = ggplot(df_to_plot, aes(Indicator,Model))  + geom_tile(aes(fill=Difference))+ geom_text(aes(label=Difference))+ scale_fill_gradientn(colors=rev(brewer.pal(9,"RdBu")),values=rescale(c(rng[1],0,rng[2])),limits=c(rng[1],rng[2]),n.breaks=8) + facet_wrap(~type,labeller=label_value) + 
